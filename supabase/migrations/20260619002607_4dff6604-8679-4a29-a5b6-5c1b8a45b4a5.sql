-- Sinaliza corridas vindas do app do cliente que ainda aguardam registro pelo operador
ALTER TABLE public.corridas
  ADD COLUMN IF NOT EXISTS aguardando_registro BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX IF NOT EXISTS idx_corridas_aguardando_registro
  ON public.corridas (aguardando_registro)
  WHERE aguardando_registro = TRUE;

-- Atualiza a função para marcar aguardando_registro = true
